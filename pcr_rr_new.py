...
def plot_extreme_segments(df, t, s, p, column_name, n=7, min_distance=2000, segment_padding=4000):
    """
    ...
    n (int): The number of highest and lowest values to find.
...
def plot_pressure_analysis(segment_df, target_pressure, threshold_percentage, deadhead_pressure, ax,t,s,p):
    ...
    # Find specific indices where the gradient is between -0.1 and 0.1 and pressure is within 1% of deadhead_pressure
    specific_indices = []
    ...
    if target_index is not None:
        ...
        #ax.scatter(target_time, target_pressure_value, color='cyan',label=f'Target Pressure ({target_pressure - threshold:.2f})')
    gradient_indices = []
    ...
    points_of_interest = {
        ...
        # 'specific_points': list(zip(specific_times, specific_pressures)),
        ...
        # 'gradient_points': list(zip(gradient_times, gradient_pressures))
...
def process_folder(folder_path):
    ...
    for file in os.listdir(folder_path):
        if file.endswith('.tdms'):
            ...
            results_df = process_file(file_path, results_folder, results_df)
    
    # Save the final results
    results_df.to_csv(os.path.join(results_folder, "all_results.csv"), index=False)
    save_beautified_excel(results_df, os.path.join(results_folder, "beautified_results.xlsx"))

def process_file(file_path, results_folder, results_df):
    print(f"Processing file: {file_path}")
    df = read_tdms(file_path)
    temp, speed, pressure = extract_speed_pressure_temperature(file_path)
    target_pressure = pressure * 0.75
    threshold_percent = get_threshold_percentage(pressure)
    deadhead_pressure = pressure
    
    segments = plot_extreme_segments(df, temp, speed, pressure, 'Outlet_Pressure_Psi', 7)
    ...
    plt.tight_layout()
    plt.suptitle(f"Plot for Temp ({temp}), Speed ({speed}), Pressure ({pressure})")
    filename = f"pc_rr_T{temp}_S{speed}_P{pressure}.png"
    plt.savefig(os.path.join(results_folder, filename), format="png", dpi=400)
    plt.close()

    for i, points in enumerate(points_list):
        new_row = create_result_row(file_path, temp, speed, pressure, i, points)
        results_df = pd.concat([results_df, pd.DataFrame([new_row])], ignore_index=True)

    return results_df

def create_result_row(file_path, temp, speed, pressure, cycle_num, points):
    return {
        "File": os.path.basename(file_path),
        "Temp": temp,
        "Speed": speed,
        "Pressure": pressure,
        "Cycle No.": cycle_num + 1,
        "Time @ Max Pressure": points['max_point'][0],
        "Pressure @ Max Pressure": points['max_point'][1],
        "Time @ Min Pressure": points['min_point'][0],
        "Pressure @ Min Pressure": points['min_point'][1],
        "Time @ Start of Response Time": points['intersection_points'][0][0],
        "Pressure @ Start of Response Time": points['intersection_points'][0][1],
        "Time @ End of Response Time": points['intersection_points'][1][0],
        "Pressure @ End of Response Time": points['intersection_points'][1][1],
        "Time @ Start of Recovery Time": points['intersection_points'][2][0],
        "Pressure @ Start of Recovery Time": points['intersection_points'][2][1],
        "Time @ End of Recovery Time": points['gradient_mean_point'][0],
        "Pressure @ End of Recovery Time": points['gradient_mean_point'][1],
        "Mean deadhead Pressure": points['deadhead_pressure_mean'],
        "Response Time": points['intersection_points'][1][0] - points['intersection_points'][0][0],
        "Recovery Time": points['gradient_mean_point'][0] - points['intersection_points'][2][0]
    }

def save_beautified_excel(df, output_file):
    grouped_df = df.groupby(['File', 'Temp', 'Speed', 'Pressure']).apply(lambda x: x).reset_index(drop=True)
...
def process_single_random_file(folder_path):
    tdms_files = [f for f in os.listdir(folder_path) if f.endswith('.tdms')]
    if not tdms_files:
        raise ValueError("No TDMS files found in the folder")
    
    random_file = random.choice(tdms_files)
    file_path = os.path.join(folder_path, random_file)
    
    single_file_results = os.path.join(folder_path, "single_file_results")
    os.makedirs(single_file_results, exist_ok=True)
    
    results_df = pd.DataFrame()
    results_df = process_file(file_path, single_file_results, results_df)
    
    results_df.to_csv(os.path.join(single_file_results, "single_file_results.csv"), index=False)
    save_beautified_excel(results_df, os.path.join(single_file_results, "single_file_beautified_results.xlsx"))

def main(folder_path):
    process_folder(folder_path)
    process_single_random_file(folder_path)
